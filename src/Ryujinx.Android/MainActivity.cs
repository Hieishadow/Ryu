...
namespace Ryujinx.Android
{
    ...
    public class MainActivity : Activity
    {
        ...
        protected override void OnCreate(Bundle? b)
        {
            ...
            romPath = System.IO.Path.Combine(baseDir, "roms");
            ...
            t.SetTextColor(Color.White); t.TextSize=11; t.SetTypeface(null, TypefaceStyle.Bold);
        ...
        void Load()
        {
            ...
            try{
                ...
                var files = Directory.GetFiles(romPath).Where(f=> f.EndsWith(".nsp")||f.EndsWith(".xci")).ToArray();
                var info = new TextView(this){ Text="Pasta: "+romPath+"\nColoque .nsp/.xci aqui" };
                ...
                if(files.Length==0){
                    ...
                    empty.SetTextColor(Color.White); empty.Gravity=GravityFlags.Center; empty.SetPadding(0,60,0,0);
                    list.AddView(empty); return;
                }
                foreach(var f in files){
                    var card = new LinearLayout(this){ Orientation=Orientation.Horizontal };
                    card.SetPadding(30,30,30,30); card.SetBackgroundColor(Color.ParseColor("#1c1c1c"));
                    ...
                    card.AddView(play);
                    play.Click+= (s,e)=> Toast.MakeText(this, "Iniciando "+System.IO.Path.GetFileName(f)+" - Core #11", ToastLength.Short).Show();
                    list.AddView(card);
                }
