...
public class GameActivity : Activity
{
    ...
    protected override void OnCreate(Bundle saved){
        ...
        if(Holder.device!=null || Holder.emuThread!=null){
            ...
            bool ended=false;
            try{ Holder.running=false; if(Holder.emuThread!=null) ended=Holder.emuThread.Join(3000); }catch{}
    ...
    public override void OnBackPressed(){
        MyLog("OnBackPressed - SAIU");
        Holder.running=false;
        try{ Holder.emuThread?.Join(2000); }catch{}
        try{ VirtualFileSystem.ResetForAndroid(); }catch{}
        base.OnBackPressed();
    }
    ...
    class CB : Java.Lang.Object, ISurfaceHolderCallback{
        ...
        public void SurfaceChanged(ISurfaceHolder h,AFormat f,int w,int ht){
            ...
            try{
                ...
                win?.GetType().GetMethod("SetSize",All)?.Invoke(win,new object[]{w,ht});
            }catch(Exception ex){ a.FileLog($"SetSize Changed FAIL {ex.Message}"); }
        }
        public void SurfaceDestroyed(ISurfaceHolder h){
            a.MyLog("SurfaceDestroyed");
            Holder.running=false;
            if(Holder.emuThread!=null){ bool ended=false; try{ ended=Holder.emuThread.Join(4000); }catch{} if(!ended) return; }
    ...
    void Emu(){
        ...
        try{
            ...
            // FIX 507 - Window v4 precisa SurfaceFrame real
            ...
            MyLog($"SetSize {w}x{h}");
            try{ var winProp=Holder.gpu.GetType().GetProperty("Window",All); var win=winProp?.GetValue(Holder.gpu); win?.GetType().GetMethod("SetSize",All)?.Invoke(win,new object[]{w,h}); MyLog($"SetSize OK {w}x{h}"); FileLog($"[VK] SetSize OK {w}x{h}"); }catch(Exception eSz){ MyLog($"SetSize ERR {eSz.Message}"); FileLog($"SetSize ERR {eSz}"); }
            ...
            while(Holder.running && Holder.nativeWindow!=IntPtr.Zero){
                try{
                    ...
                    // FIX 507 - callback obrigatório pro Window v4 logar Present OK
                    Holder.device.PresentFrame(()=>{
                        if(frames % 60 == 0){
                            FileLog($"[VK] swapBuffers cb f={frames} OK");
                        }
                    });
                    frames++;
                    if(frames % 60 == 0){ FileLog($"[FILE] LOOP f={frames} PF/PR OK"); }
                    Thread.Sleep(16);
