...
public class GameActivity : Activity
{
    ...
    void Emu(){ try{
        ...
        try{
            ...
            if(mInit!=null){ var pars=mInit.GetParameters(); if(pars.Length==0) mInit.Invoke(gpu,null); else mInit.Invoke(gpu,new object[]{0}); MyLog("Vulkan Initialize() OK"); }
        }catch(Exception ex){ MyLog("Vulkan Init fail: "+(ex.InnerException?.Message??ex.Message)); }
