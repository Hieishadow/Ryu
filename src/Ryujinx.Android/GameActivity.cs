...
public class GameActivity : Activity
{
    ...
    public override void OnBackPressed(){ MyLog("OnBackPressed - stopping"); running=false; try{ if(emuThread!=null && emuThread.IsAlive){ if(!emuThread.Join(3000)) MyLog("Back Join TIMEOUT id="+emuThread.ManagedThreadId); else MyLog("Back Join OK"); } }catch{} base.OnBackPressed(); }
    class CB : Java.Lang.Object, ISurfaceHolderCallback{
        ...
        public void SurfaceCreated(ISurfaceHolder h){
            var r=h.SurfaceFrame; if(r.Width()<=0) return; if(a.emuThread!=null && a.emuThread.IsAlive){ a.MyLog($"SurfaceCreated ignorando, thread viva id={a.emuThread.ManagedThreadId}"); return; }
            a.MyLog($"Surface {r.Width()}x{r.Height()} tid={SysEnv.CurrentManagedThreadId}");
        ...
        public void SurfaceDestroyed(ISurfaceHolder h){
            a.MyLog("SurfaceDestroyed START");
            a.running=false;
            bool ended=true;
            if(a.emuThread!=null){
                a.MyLog($"Waiting EmuThread id={a.emuThread.ManagedThreadId}...");
                try{ ended=a.emuThread.Join(5000); }catch(Exception ex){ a.MyLog("Join ERROR "+ex.Message); ended=false; }
                if(!ended){ a.MyLog($"WARNING: EmuThread id={a.emuThread.ManagedThreadId} NAO TERMINOU - retorno sem liberar"); return; }
                a.MyLog("EmuThread terminou OK");
            }
            try{ a.MyLog($"SWITCH DISPOSE hash={a.device?.GetHashCode()} gpuType={a.gpu?.GetType().FullName} isDisp={a.gpu is IDisposable}"); }catch{}
            a.MyLog("ANTES Dispose device");
            try{ a.device?.Dispose(); a.MyLog("Dispose device OK"); }catch(Exception ex){ a.MyLog("Dispose device ERROR "+ex); }
            a.device=null;
            try{ if(a.gpu is IDisposable d){ d.Dispose(); a.MyLog("Dispose gpu OK"); } }catch(Exception ex){ a.MyLog("Dispose gpu ERROR "+ex); }
            a.gpu=null;
            if(a.nativeWindow!=IntPtr.Zero){ try{ ANativeWindow_release(a.nativeWindow); a.MyLog("ANW release OK"); }catch{} a.nativeWindow=IntPtr.Zero; }
            a.MyLog("SurfaceDestroyed END");
        }
    }
    class DummyUI : IHostUIHandler
    {
        public bool DisplayMessage(string title, string message){ return true; }
        public bool DisplayInput(HostUIInputConfig c){ return true; }
        public void DisplayErrorApplet(ulong a, ulong b, string[] c, string d, string e){}
        public IDialogParent CreateDialogParent(){ return new DummyDialog(); }
        public bool DisplayChildDialog(IHostUIHandler d){ return true; }
    }
    class DummyDialog : IDialogParent { public void Show(){} public void Hide(){} public void SetDialog(IHostUIHandler d){} }

    void Emu(){
        int tid=SysEnv.CurrentManagedThreadId;
        MyLog($"Emu THREAD START id={tid}");
        try{
            MyLog($"Emu START tid={tid}");
            string baseDir=Path.Combine(FilesDir.AbsolutePath,"Ryujinx");
            Directory.CreateDirectory(Path.Combine(baseDir,"system")); Directory.CreateDirectory(Path.Combine(baseDir,"keys"));
            ...
            try{ var dk=Path.Combine(baseDir,"keys","prod.keys"); if(!File.Exists(dk)){ foreach(var s in new[]{ "/storage/emulated/0/Ryubing/keys/prod.keys", "/storage/emulated/0/Download/Ryubing/keys/prod.keys" }) if(File.Exists(s)){ File.Copy(s,dk,true); break; } } MyLog($"keys {new FileInfo(dk).Length}"); }catch{}
            ...
            MyLog($"SWITCH CREATED hash={device.GetHashCode()} tid={tid}");
            ...
            while(running && nativeWindow!=IntPtr.Zero){
                try{
                    MyLog($"Frame {frames} Process START hash={device.GetHashCode()} tid={tid}");
                    device.ProcessFrame();
        ...
        }catch(Exception ex){ MyLog($"CRASH tid={tid} {ex}"); }
        finally{ MyLog($"Emu THREAD END id={tid}"); }
    }
    ...
    HleConfiguration BuildHle(VirtualFileSystem vfs, VulkanRenderer gpu, DummyHardwareDeviceDriver audio, string baseDir, string sysDir){
        ...
        try{ var prop = hleType.GetProperties(All).FirstOrDefault(p=>p.PropertyType.Name.Contains("UI")); prop?.SetValue(hle, new DummyUI()); }catch{}
        var confM=hleType.GetMethod("Configure",All); var cps=confM.GetParameters(); var cargs=new object[cps.Length];
        for(int k=0;k<cps.Length;k++){ var pt=cps[k].ParameterType; if(pt==typeof(VirtualFileSystem)) cargs[k]=vfs; else if(pt==typeof(LibHacHorizonManager)) cargs[k]=lhm; else if(pt==typeof(ContentManager)) cargs[k]=cm; else if(pt==typeof(AccountManager)) cargs[k]=accMan; else if(pt==typeof(UserChannelPersistence)) cargs[k]=ucp; else if(pt.IsAssignableFrom(gpu.GetType())) cargs[k]=gpu; else if(pt.FullName.Contains("IRenderer")) cargs[k]=gpu; else if(typeof(IHardwareDeviceDriver).IsAssignableFrom(pt)) cargs[k]=audio; else if(typeof(IHostUIHandler).IsAssignableFrom(pt)) cargs[k]=new DummyUI(); }
