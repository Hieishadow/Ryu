    ...
    class MiiDatabaseManager
    {
        ...
        public int GetCount(DatabaseSessionMetadata metadata)
        {
            if (!metadata.MiiKeyCode.IsEnabledSpecialMii())
            {
                ...
                {
                    StoreData tmp = _database.Get(i);
                    if (!tmp.IsSpecial()) count++;
                }
                return count;
            }
            else return _database.Length;
        }
        public void Get(DatabaseSessionMetadata metadata, int index, out StoreData storeData)
        {
            if (!metadata.MiiKeyCode.IsEnabledSpecialMii())
            {
                if (GetAtVirtualIndex(index, out int realIndex, out _)) index = realIndex;
                else index = 0;
            }
            storeData = _database.Get(index);
        }
        public ResultCode FindIndex(DatabaseSessionMetadata metadata, out int index, CreateId createId) => FindIndex(out index, createId, metadata.MiiKeyCode.IsEnabledSpecialMii());
        public ResultCode FindIndex(out int index, CreateId createId, bool isSpecial)
        {
            ...
            {
                if (isSpecial) { index = realIndex; return ResultCode.Success; }
                StoreData storeData = _database.Get(realIndex);
                if (!storeData.IsSpecial())
                {
                    if (realIndex < 1) index = 0;
                    else index = ConvertRealIndexToVirtualIndex(realIndex);
                    return ResultCode.Success;
                }
            }
            index = -1;
        ...
        public ResultCode Move(DatabaseSessionMetadata metadata, int newIndex, CreateId createId)
        {
            if (!metadata.MiiKeyCode.IsEnabledSpecialMii())
            {
                if (GetAtVirtualIndex(newIndex, out int realIndex, out _)) newIndex = realIndex;
                else newIndex = 0;
            }
            if (_database.GetIndexByCreatorId(out int oldIndex, createId))
            {
                StoreData realStoreData = _database.Get(oldIndex);
                if (!metadata.MiiKeyCode.IsEnabledSpecialMii() && realStoreData.IsSpecial()) return ResultCode.InvalidOperationOnSpecialMii;
                ResultCode result = _database.Move(newIndex, oldIndex);
                if (result == ResultCode.Success) MarkDirty(metadata);
                return result;
            }
            return ResultCode.NotFound;
        }
        ...
        {
            if (!storeData.IsValid()) return ResultCode.InvalidStoreData;
            if (!metadata.MiiKeyCode.IsEnabledSpecialMii() && storeData.IsSpecial()) return ResultCode.InvalidOperationOnSpecialMii;
            if (_database.GetIndexByCreatorId(out int index, storeData.CreateId))
            {
                StoreData oldStoreData = _database.Get(index);
                if (oldStoreData.IsSpecial()) return ResultCode.InvalidOperationOnSpecialMii;
                _database.Replace(index, storeData);
            }
            else
            {
                if (_database.IsFull()) return ResultCode.DatabaseFull;
        ...
        {
            if (!charInfo.IsValid()) return ResultCode.InvalidCharInfo;
            if (charInfo.Type == 1) return ResultCode.InvalidOperationOnSpecialMii;
            CoreData coreData = new();
            coreData.SetFromCharInfo(charInfo);
            StoreData storeData;
            do { storeData = StoreData.BuildFromCoreData(utilityImpl, coreData); }
        ...
        {
            if (!_database.GetIndexByCreatorId(out int index, createId)) return ResultCode.NotFound;
            if (!metadata.MiiKeyCode.IsEnabledSpecialMii())
            {
                StoreData storeData = _database.Get(index);
                if (storeData.IsSpecial()) return ResultCode.InvalidOperationOnSpecialMii;
            }
        ...
        {
            if (_isDirty) return (ResultCode)ForceSaveDatabase().Value;
            else return ResultCode.NotUpdated;
        }
        public void FormatDatabase(DatabaseSessionMetadata metadata) { _database.Format(); MarkDirty(metadata); }
        public bool IsFullDatabase() => _database.IsFull();
    }
}
